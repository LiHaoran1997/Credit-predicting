...
def read_data():
    ...
    :return: 输入值和标签
    '''
    file_path = 'data.xlsx'   #路径
    data = pd.read_excel(file_path,sheet_name='cpu')  #读取
    attribute_list = ['cpu', 'memory']  # 通过这两个属性来预测
    df=data.iloc[:, 1:8]#1-8列
    X = df.loc[:, attribute_list].values
    X1 = df.loc[:, 'cpu'].values#‘cpu’列数据
...
def main():
    ...
    print('系数为:',clf.coef_)
    #xjx
    # X_continous_logistic = linear_model.LogisticRegression(penalty='l2',c=1.0,solver='lbfgs',max_iter=500)  # 逻辑回归
    # X_continous_logistic.fit(X_train,y_train)
    # parameters=X_continous_logistic.coef
